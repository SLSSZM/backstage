<template>
  <div>
    ...
    <el-card>
      ...
      <el-table :data="rolesList" border stripe>
        ...
        <el-table-column type="expand">
          <template v-slot="scope">
            <el-row :class="['dbBottom', index1 === 0 ? 'dbTop' : '', 'vcenter']"
              ...
              <!-- 渲染一级权限 -->
              <el-col :span="5">
                <el-tag type="parmary" closable @close="removeRightById(scope.row, item1.id)">{{item1.authName}}
                </el-tag>
                <i class="el-icon-caret-right"></i>
              </el-col>
              <el-col :span="19">
                <!-- 渲染二级权限 -->
                <el-row :class="[index2 === 0 ? '' : 'dbTop', 'vcenter']" v-for="(item2, index2) in item1.children"
                  :key="item2.id">
                  <el-col :span="6">
        ...
        <el-table-column label="操作">
          <template v-slot="scope">
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditDialog(scope.row._id)">编辑
            </el-button>
            <el-button type="danger" icon="el-icon-delete" size="mini" @click="showDeleteDialog(scope.row._id)">删除
            </el-button>
    ...
    <!-- 添加角色对话框 -->
    <el-dialog title="添加角色" :visible.sync="addDialogVisible" @close="addDialogColse" width="50%">
      <el-form :model="addRoleFrom" :rules="addRoleRules" ref="addRoleRef">
    ...
    <!-- 编辑角色对话框 -->
    <el-dialog title="编辑角色" :visible.sync="editDialogVisible" @close="editDialogColse" width="50%">
      <el-form :model="editRoleFrom" :rules="editRoleRules" ref="editRoleRef">
        <el-form-item label="角色名称" prop="roleName">
    ...
    <!-- 分配权限对话框 -->
    <el-dialog title="分配权限" :visible.sync="setRightsDialog" width="50%" @close="setRightsDialogClose">
      <!-- 树形控件 -->
      <el-tree :data="setRightsList" :props="treeProps" show-checkbox default-expand-all node-key="id"
